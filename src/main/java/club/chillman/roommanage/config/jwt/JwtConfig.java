package club.chillman.roommanage.config.jwt;
...
public class JwtConfig {
     ...
     * JWT 自定义密钥 我这里写死的
     */
    private static final String SECRET_KEY = "6471f568a45e5ab3f442c38e7932aef24447131f";
     ...
     *
     * @param wxAccountDTO 微信用户信息
     * @return 返回 jwt token
     */
    public String createTokenByWxAccount(WxAccountDTO wxAccountDTO) {
        String jwtId = UUID.randomUUID().toString();                 //JWT 随机ID,做为验证的key
        //1 . 加密算法进行签名得到token
        Algorithm algorithm = Algorithm.HMAC256(SECRET_KEY);
        String token = JWT.create()
                .withClaim("wxOpenId", wxAccountDTO.getWxOpenid())
                .withClaim("userId", wxAccountDTO.getUserId())
                .withClaim("sessionKey", wxAccountDTO.getSessionKey())
                .withClaim("jwt-id", jwtId)
                .withExpiresAt(new Date(System.currentTimeMillis() + EXPIRE_TIME * 1000))  //JWT 配置过期时间的正确姿势
                .sign(algorithm);
        //2 . Redis缓存JWT,和JWT过期时间一致
    ...
    public boolean verifyToken(String token) {
        try {
            ...
            if (StringUtils.isEmpty(redisToken)) throw new RoomManageException(422, "token过期");;
            ...
            System.out.println(getUserIdByToken(redisToken)+"验证通过");
            return true;
